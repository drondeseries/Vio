...
describe("watchDetailFromV2", () => {
  it("adapts the getWatchState fixture back to the player's WatchDetail", () => {
    const detail = watchDetailFromV2(
      v2Fixture<"GET /api/v2/watch/{id}">(getWatchStateOk as components["schemas"]["WatchDetail"]),
    );
    ...
    expect(version?.marker_segments).toEqual(getWatchStateOk.versions[0]?.marker_segments);
