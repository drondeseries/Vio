...
/** Stages content in a host-pick lobby without starting playback. */
export async function stageRoomItem(
  roomId: string,
  ...
  const result = await v2("PUT /api/v2/watch-together/rooms/{room_id}/staged-selection", {
    path: { room_id: roomId },
    body: {
