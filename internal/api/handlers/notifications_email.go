...
// devices that may have no Vio session, so they render minimal standalone
...
func (h *EmailLinkHandler) HandleVerify(w http.ResponseWriter, r *http.Request) {
	...
	case outcome == notifications.EmailVerifyConflict:
		writeEmailLinkPage(w, http.StatusConflict, "Address already in use",
			"This address now belongs to another profile or account. Choose a different address in Vio's notification settings.")
	case outcome == notifications.EmailVerifyInvalid:
		writeEmailLinkPage(w, http.StatusBadRequest, "Link expired or already used",
			"Request a new verification email from Vio's notification settings.")
	default:
		writeEmailLinkPage(w, http.StatusOK, "Address verified",
			"Vio notifications for this profile will now be delivered here. You can close this page.")
	}
}
...
func (h *EmailLinkHandler) HandleUnsubscribe(w http.ResponseWriter, r *http.Request) {
	...
	case !ok:
		writeEmailLinkPage(w, http.StatusBadRequest, "Link invalid",
			"This unsubscribe link is no longer valid. Manage notifications in Vio's settings.")
	default:
		writeEmailLinkPage(w, http.StatusOK, "Unsubscribed",
			"This profile will no longer receive notification emails. Re-enable them any time in Vio's settings.")
	}
}
...
func writeEmailLinkPage(w http.ResponseWriter, status int, title, detail string) {
	...
	_, _ = fmt.Fprintf(w, `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>%s — Vio</title></head>
