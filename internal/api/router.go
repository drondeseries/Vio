...
func newChiRouter(deps Dependencies) chi.Router {
	...
	if deps.DB != nil {
		...
		detailSvc.SetVirtualRankingSource(virtualCandidateRankingSource(deps.VirtualLibraryService))
	...
	if deps.DB != nil && deps.S3Public != nil {
		...
		subtitleManager = subtitles.NewManager(subtitleRepo, blobstore.NewByteStore(blobstore.NewS3(deps.S3Public)))
	...
	if deps.DB != nil {
		...
		// service there is no variants provider and virtual syncs fail
		// explicit (provider unavailable), never a silent downgrade.
			...
			r.Group(func(r chi.Router) {
				...
				if ebookReaderHandler != nil {
					r.Route("/ebooks", func(r chi.Router) {
						...
						// The page/file delivery carries stream telemetry like
						// the other media egress routes.
				...
				if playbackHandler != nil {
					...
					r.Route("/playback", func(r chi.Router) {
						...
						// pattern as /stream/{session_id}. Both delivery
						// routes carry stream telemetry like the /stream and
						// download media routes below.
				...
				// Stream routes. The observeNative wrapper enrolls each route
				// in stream telemetry; media_routes.go declares these
				// method+pattern pairs, so dropping the wrapper would strand
				// the declaration.
				if streamHandler != nil {
